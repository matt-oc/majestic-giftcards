...
const d = debug_1.default('electron-rebuild');
...
async function getClangEnvironmentVars(electronVersion, targetArch) {
    ...
    if (process.platform === 'linux') {
        const sysrootPath = await sysroot_fetcher_1.downloadLinuxSysroot(electronVersion, targetArch);
...
async function downloadClangVersion(electronVersion) {
    ...
    const electronDeps = await fetcher_1.fetch(`https://raw.githubusercontent.com/electron/electron/v${electronVersion}/DEPS`, 'text');
    ...
    const base64ClangUpdate = await fetcher_1.fetch(`https://chromium.googlesource.com/chromium/src.git/+/${chromiumRevision}/tools/clang/scripts/update.py?format=TEXT`, 'text');
    ...
    const contents = await fetcher_1.fetch(clangDownloadURL, 'buffer');
    d('deflating clang');
